...
#         # 2. Logic to distinguish Seed Size from Purity
#         if len(parts) > 4:
#             text5 = parts[4].strip()
#             # If it looks like XX.XX, check value to decide if it's Purity or Size
#             if re.fullmatch(r"\d+\.\d{2}", text5):
#                 val = float(text5)
#                 if val > 50: 
#                     # Likely purity (e.g. 99.99)
#                     purity = val
#                 else:
#                     # Likely seed size (e.g. 4.75)
#                     seed_size = text5
#             else:
#                 # Not XX.XX format, likely seed size (e.g. 5.5, 12, etc)
#                 seed_size = text5

#         # If we identified seed_size above, check if it contains a purity-like value embedded
#         # (Though usually it would be cleaner from the logic above)
#         if seed_size:
...
#         # Fallback: if we haven't found purity yet, check the next line
    ...
    for fname, bts in pdf_files:
        try:
            ...
            for page in doc:
                t = page.get_text()
                # ── FIX 3 ──────────────────────────────────────────────────────────────
                # OLD: PyMuPDF sometimes splits the table header across lines as
                #      "Purity\nAnalysis", so the plain string check misses it.
                #      Also add "Lot Number:" as a robust fallback anchor that every
                #      seed analysis report contains regardless of layout.
                # if "Purity Analysis" in t:
                #     full_text += t + "\n"
...
def extract_invoice_from_pdf(
    ...
    try:
        ...
        while i < len(all_blocks):
            ...
            # ── FIX 1a ─────────────────────────────────────────────────────────────
            # OLD: txt = b[4].strip()
            # Normalize any intra-block newlines so "PERFORMAX®\nTreated 100M"
            # becomes "PERFORMAX® Treated 100M" in a single string.
            # OLD: txt = b[4].strip()
            ...
            # ── FIX 1b ─────────────────────────────────────────────────────────────
            # OLD: single-line check that misses "Treated"/"Untreated" on a wrapped
            #      continuation block (e.g. INV-191194 "PERFORMAX® Treated 100M").
            # OLD:
            # if re.match(r"^\d+\s+\d{8}", txt) and ("Treated" in txt or "Untreated" in txt):
            #
            # NEW: first confirm the block starts an item row (line number + 8-digit
            #      item number), then scan up to 5 following blocks for the keyword
            #      before deciding this is truly an item line.
            is_item_line = False
            if re.match(r"^\d+\s+\d{8}", txt):
                lookahead = txt
                for _k in range(i + 1, min(i + 6, len(all_blocks))):
                    _kt = all_blocks[_k][4].strip().replace("\n", " ")
                    if re.match(r"^\d+\s+\d{8}", _kt):
                        break  # next item row starts — stop looking
                    lookahead += " " + _kt
                if "Treated" in lookahead or "Untreated" in lookahead:
                    is_item_line = True
            # ── END FIX 1b ─────────────────────────────────────────────────────────

            # ── FIX 1c ─────────────────────────────────────────────────────────────
            # OLD: if re.match(r"^\d+\s+\d{8}", txt) and ("Treated" in txt or "Untreated" in txt):
            if is_item_line:
            # ── END FIX 1c ─────────────────────────────────────────────────────────
                if current:
                    treatment_name = re.search(r"Treatment name:\s*(.*)", text_acc)
                ...
                while j < len(all_blocks) and abs(all_blocks[j][1] - item_y0) < 10:
                    # ── FIX 1d ─────────────────────────────────────────────────────
                    # OLD: item_line_acc += " " + all_blocks[j][4].strip()
                    item_line_acc += " " + all_blocks[j][4].strip().replace("\n", " ")
                    # ── END FIX 1d ─────────────────────────────────────────────────
                    j += 1

                # ── FIX 1e ─────────────────────────────────────────────────────────
                # NEW: if "Treated"/"Untreated" is still absent from item_line_acc
                #      after collecting the same horizontal line, the description has
                #      wrapped onto the very next block row (e.g. INV-191194).
                #      Grab that continuation line and splice it before "EA" so that
                #      desc parsing and pkg_qty extraction work correctly.
                if "Treated" not in item_line_acc and "Untreated" not in item_line_acc and j < len(all_blocks):
                    next_y = all_blocks[j][1]
                    cont_parts = []
                    k2 = j
                    while k2 < len(all_blocks) and abs(all_blocks[k2][1] - next_y) < 10:
                        ct = all_blocks[k2][4].strip().replace("\n", " ")
                        if re.match(r"^\d+\s+\d{8}", ct):
                            break  # stop if we've hit the next item row
                        cont_parts.append(ct)
                        k2 += 1
                    cont_txt = " ".join(cont_parts)
                    if "Treated" in cont_txt or "Untreated" in cont_txt:
                        # Insert before "EA" so the description includes "Treated XxxM"
                        if " EA " in item_line_acc:
                            item_line_acc = item_line_acc.replace(" EA ", f" {cont_txt} EA ", 1)
                        else:
                            item_line_acc += " " + cont_txt
                        j = k2  # advance past the wrapped continuation line
                # ── END FIX 1e ─────────────────────────────────────────────────────
    ...
    for filename, pdf_bytes in pdf_files:
        ...
        try:
            ...
            with fitz.open(stream=pdf_bytes, filetype="pdf") as temp_doc:
                ...
                # ── FIX 2 ──────────────────────────────────────────────────────────
                # OLD: exclusion-based check — misses split "Purity\nAnalysis" text
                #      and has no guard against packing slips being treated as invoices.
                # OLD:
                # if "Report of Seed Analysis" in full_doc_text or "Purity Analysis" in full_doc_text:
                #     is_invoice = False
                #     logger.info(f"Identified as analysis report/supporting doc; skipping invoice processing for: {filename}")
                #
                # NEW (v2): Three-way check that correctly distinguishes:
                #   A) Image-based PDFs (scanned invoices with stamps but no text layer)
                #      → PyMuPDF extracts <200 chars (just rubber stamps).
                #      → Cannot be processed without OCR. Skip with a clear warning.
                #   B) Known non-invoice supporting documents (packing lists, seed analysis)
                #      → Have substantial text but contain identifying phrases.
                #      → Skip silently.
                #   C) Normal text-layer Sakata invoices
                #      → Contain "INV-" in their text layer. → Process normally.
                #   D) Substantial text but no "INV-" — unusual, attempt anyway.
                #
                # ROOT CAUSE NOTE for case A: INV-191194 is a scanned/image-based PDF.
                # PyMuPDF only finds the rubber-stamp text layer (BRD 6268, 1200084T,
                # 36-1593-100MS) — 32 chars total. The actual invoice content is stored
                # as a JPEG image (Im0 XObject) with no underlying text. OCR is required.
                #
                # OLD (v1 — grouped image PDFs with non-invoices, wrong skip message):
                # is_invoice = "INV-" in full_doc_text
                # if not is_invoice:
                #     logger.info(f"Skipping non-invoice file (no 'INV-' found): {filename}")

                text_len = len(full_doc_text.strip())
                _NON_INVOICE_PHRASES = [
                ...
                if text_len < 200:
                    # Case A: Image-based / scanned PDF — no usable text layer.
                    is_invoice = False
                    logger.warning(
                        f"Skipping image-based PDF '{filename}': only {text_len} chars "
                        f"extracted (stamps only). This invoice requires OCR to process. "
                        f"Please obtain the digital/text-layer version from Sakata."
                    )
                elif any(phrase in full_doc_text for phrase in _NON_INVOICE_PHRASES):
                    # Case B: Supporting document (seed analysis report or packing slip).
                    is_invoice = False
                    logger.info(f"Skipping supporting document (non-invoice): {filename}")
                elif "INV-" in full_doc_text:
                    # Case C: Normal Sakata invoice with a full text layer.
                    is_invoice = True
                else:
                    # Case D: Has substantial text but no "INV-" — unusual, attempt anyway.
                    is_invoice = True
                    logger.warning(
                        f"No 'INV-' found in '{filename}' but text layer is present "
                        f"({text_len} chars). Attempting extraction — verify results."
                    )
                # ── END FIX 2 ──────────────────────────────────────────────────────
