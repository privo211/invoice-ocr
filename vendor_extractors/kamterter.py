...
def extract_kamterter_data_from_bytes(pdf_files: list[tuple[str, bytes]]) -> dict[str, list[dict]]:
    ...
    for filename, pdf_bytes in pdf_files:
        ...
        for block in ktt_blocks:
            ...
            # Robust Numeric PO Check (Remove spaces)
            ...
            # --- Item Logic (FIXED SEED TYPE) ---
            seed_type = "Unknown"
            if m_seed := re.search(r"Seed\s*Type[\s:]*(\S[^\n]*)", block, re.IGNORECASE):
                raw_seed = m_seed.group(1).strip()
                # FIX 2: Split by "Lot #" to stop capturing garbage if it's on the same line
            ...
            # 2. Try finding 'Subtotal: $XXX.XX'
                ...
                subtotal = parse_currency(m_sub_fwd.group(1))
            # 3. Fallback: Find ALL prices and pick the last valid one
            elif not subtotal:
                prices = re.findall(r"\$([\d,]+\.\d{2})", block)
                if prices:
                    subtotal = parse_currency(prices[-1])

            # FIX 3: Prevent capturing the $20.00 "Work Order Split" as the subtotal
            # If subtotal is exactly 20.00, and we see other prices, it's likely wrong.
            if abs(subtotal - 20.00) < 0.01:
                all_prices_raw = re.findall(r"\$([\d,]+\.\d{2})", block)
                # Filter out 20.00 and 0.00 to find the real line total
