    ...
    for filename, pdf_bytes in pdf_files:
        ...
        try:
            ...
            with fitz.open(stream=pdf_bytes, filetype="pdf") as temp_doc:
                ...
                #     logger.info(f"Identified as analysis report/supporting doc; skipping invoice processing for: {filename}")
                #
                # NEW: inclusion-based check — only real Sakata invoices carry "INV-".
                #      Packing slips and seed analysis reports do not, so both are
                #      correctly excluded in a single reliable condition.
